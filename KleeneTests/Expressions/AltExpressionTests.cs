...
namespace KleeneTests
{
    public class AltExpressionTests
    {
        ...
        public class RunOverload
        {
            ...
            {
                // Given
                var item1 = new RunnableExpression<char>(c1);
                var item2 = new RunnableExpression<char>(c2);
                var expression = new AltExpression<Runnable<char>>(new[] { item1, item2 });
                var input = new Runnable<char>(c1);
            ...
            {
                // Given
                var expression = new AltExpression<ObjectSequence<Runnable<char>>>(new[] {
                    c1.ToString().ToRunnable().ToExpression(),
                    c2.ToString().ToRunnable().ToExpression(),
                    c3.ToString().ToRunnable().ToExpression(),
                });
                var input = "".ToRunnable();
            ...
            public void TwoChoicesNoMatch_ReturnsEmpty(char c1, char c2, char c)
            {
                ...
                var input = c.ToString().ToRunnable();
            ...
            public void OneChoiceMatch_ReturnsChoice(char c)
            {
                // Given
                var expression = new AltExpression<ObjectSequence<Runnable<char>>>(new[] {
                    c.ToString().ToRunnable().ToExpression()
                });
                var input = c.ToString().ToRunnable();
